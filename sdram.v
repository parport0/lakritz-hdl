module sdram (
	...
	// 37:25 -- row
	// 24:16 -- column
	// 15:0 -- data to write when writing
	...
	// "speed grades". I failed timing at 166 MHz, so I am running half that.
	// I set the SDRAM mode to "CL3" (CAS Latency = 3) (I can probably set it to CL2?)
	// and that one clock cycle would be around ~12 nS (83 MHz = 12.048 nS)
	...
	// function the same. It makes looking info up that much easier.

	// Pins:
	// CLK  -- clock input (clock driven by the FPGA)
	assign dram_clk = clk_83;
	...
	// 200 uS = 200_000 nS = 16_667 clock cycles
	reg [15:0] pause_counter = 0;
	reg [10:0] auto_refresh_counter = 0;

	// A separate register and flag are needed here because dram_dq is inout
	...
	always @(negedge clk_83) begin
		if (state == 0) begin
			...
			if (pause_counter == 16680) begin // 200uS
				state <= state + 1;
				pause_counter <= 0;
			end else begin
	...
	// t_RP must pass after this (15 nS = 2 clock cycles)
		if (state == 1) begin
			auto_refresh_counter <= auto_refresh_counter + 1;
	...
	// Diagram 10.4 "Mode Register Set Cycle"
		if (state == 2) begin
			...
			if (pause_counter == 0) begin
				...
				dram_a[6:4] <= 3'b011; // CAS latency = 3
	...
	// Eight commands that are t_RC (60 nS = 6 clock cycles) clocks apart
		if (state == 3) begin
			auto_refresh_counter <= auto_refresh_counter + 1;
			...
			if (pause_counter[7:0] == 8'b0111_0101) begin // 8 times 6
				state <= state + 1;
				pause_counter <= 0;
			end else if (pause_counter[3:0] == 5) begin
				pause_counter[3:0] <= 0;
	...
	// that 8192 refresh cycles are needed every 64 mS (5_333_334 clock cycles)
	// They specify 8192 because that's 2^13 = number of rows in every bank!
	// (remember, the row is specified with A0-A12?)

	// This does not really mean that an auto-refresh is needed every 651
	...
	// must wait t_RP (15 nS = 2 clock cycles).
	// Before giving an autorefresh command and any other command, you
	// must wait t_RC ((60 nS = 6 clock cycles).
	// (that's from the 11.11 timing diagram)
	...
	// refreshed on a given cycle.
		...
		if (state == 4) begin
			...
			if (auto_refresh_counter > 512) begin
				state <= 7;
				pause_counter <= 0;
	...
	// the read or write command
		if (state == 5) begin
			...
			if (pause_counter == 2) begin // t_RCD
				state <= state + 1;
				pause_counter <= 0;
	...
	// = 6 clock cycles
	// 3 clock cycles (t_RCD) have already been waited for
	...
	// My CAS Latency is, again, 3
		...
		if (state == 6) begin
			...
			if (pause_counter == 5) begin // t_RC - t_RCD
				state <= 4;
				pause_counter <= 0;
			end else if (pause_counter == 3) begin
				dram_dq_reg <= dram_dq;
				pause_counter <= pause_counter + 1;
			end else begin
				pause_counter <= pause_counter + 1;
